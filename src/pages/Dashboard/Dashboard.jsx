...
export default function Dashboard() {
  ...
  const [isModalOpenAff, setIsModalOpenAff] = useState(false);
  ...
  const openModalAffLink = () => {
    setIsModalOpenAff(true);
  };

  const closeModalAffLink = () => {
    setIsModalOpenAff(false);
  };
  return (
    <div className="w-full h-auto bg-[#f5f5f5]">
      <div className="mx-auto max-w-[1600px] flex">
        ...
        {isModalOpenAff && (
          <>
            ...
            <div className="bg-white flex flex-col fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 w-[90%] md:w-[60%] lg:w-[40%] xl:w-[30%] 2xl:w-[25%] px-8 py-2 rounded-lg gap-8">
              ...
              <div className="flex items-center justify-center">
                <button className="bg-green-400 text-white w-full py-3 rounded-lg font-semibold tracking-wide opacity-80">
                  Generate Smartlink
                </button>
              </div>
              <div className="text-sm px-10 text-center mb-10">
                By Generating Afflink , You agree to OpeninApp’s
                <span className="text-blue-500 cursor-pointer">
                  {" "}
                  Terms of services{" "}
                </span>
        ...
        <RightBarDash
          ...
          openModalAffLink={openModalAffLink}
